...
class LinearTable{
	constructor() {
		this.head = new LinearNode(0, 'head', null); // 链表头节点
	...
	initialize(length) {
		...
		for(let j = 1;j<=this.length;++j){
			const newNode = new LinearNode(j,0,this.head.next);
	...
	addNode(data,id=0){
		...
		const newNode = new LinearNode(this.length,data,node_ptr.next);

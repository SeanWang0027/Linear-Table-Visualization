...
class CircularTable{
	constructor() {
		this.head = new CircularNode(0, 'head', null); // 链表头节点
	...
	initialize(length) {
		...
		for(let j = 1;j<=this.length;++j){
			const newNode = new CircularNode(j,0,this.head.next);
	...
	addNode(data,id=0){
		...
		const newNode = new CircularNode(this.length,data,node_ptr.next);
